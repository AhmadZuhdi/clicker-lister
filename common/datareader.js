 ...
 */
function readRelic(data, gameData) {
	...
	$.each(data.items, function() {
		...
		if(this.bonusType1){
				...
				var type1 = gameData.itemBonusTypes[this.bonusType1]
				type1.key = 1
				type1.bonusValue = this.bonus1Level
				times.push(type1)
		}

		if(this.bonusType2) {

				var type2 = gameData.itemBonusTypes[this.bonusType2]
				type2.key = 2
				type2.bonusValue = this.bonus2Level
				times.push(type2)
		}

		if(this.bonusType3) {

			var type3 = gameData.itemBonusTypes[this.bonusType3]
			type3.key = 3
			type3.bonusValue = this.bonus3Level
			times.push(type3)
		}

		if(this.bonusType4) {

			var type4 = gameData.itemBonusTypes[this.bonusType4]
			type4.key = 4
			type4.bonusValue = this.bonus4Level
			times.push(type4)
		}
		...
		$.each(times, function() {
			...
			var bonusValue = this.bonusValue
			var scaledBonusValue = bonusValue

			switch (this.levelAmountFormula) {
				case "linear0_25":
					scaledBonusValue = bonusValue * 0.25;
					break;
				case "linear5":
					scaledBonusValue = bonusValue * 5;
					break;
				case "linear10":
					scaledBonusValue = bonusValue * 10;
					break
				case "linear15":
					scaledBonusValue = bonusValue * 15;
					break
				case "linear25":
					scaledBonusValue = bonusValue * 25;
					break;
				case "libAndSiy":
					scaledBonusValue = IdleValue(bonusValue);
					break;
				case "solomonRewards":
					scaledBonusValue = SolomonValue(bonusValue);
					break;

			}

			var effect = this.effectDescription.replace(/%1/g, scaledBonusValue)
			currentOutput+= "\n" + effect
			// currentOutput+= "---"
